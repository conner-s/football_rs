...
fn draw_charts(f: &mut Frame, app: &mut App, area: Rect) {
        ...
        .split(area);
    {
            ...
            .split(chunks[0]);
        {
            ...
            // Draw tasks
            let players: Vec<ListItem> = app
                .players
                .items
                .iter()
                .map(|player| ListItem::new(vec![text::Line::from(Span::raw(player.person().first_name()))]))
                .collect();
            let players = List::new(players)
                .block(Block::default().borders(Borders::ALL).title("Players"))
                .highlight_style(Style::default().add_modifier(Modifier::BOLD))
                .highlight_symbol("> ");
            f.render_stateful_widget(players, chunks[0], &mut app.players.state);
